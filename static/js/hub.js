(() => {
  ...
  // Legacy product-plate showcase removed; the collection viewer below is the
  // single source for 3D inspection on the Hub.
  ...
  function startCollectionShowcase() {
    ...
    let renderer;
    try { renderer = new THREE.WebGLRenderer({ canvas, antialias: !/Mobi/i.test(navigator.userAgent), alpha: true }); } catch { return; }
    const scene = new THREE.Scene();
