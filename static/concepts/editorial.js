/* ==========================================================================
   CapyCrew redesign concept 02 - "The CapyCity Dispatch"
   Editorial direction. Behaviour is deliberately quiet: type leads, the page
   never moves under the reader, and every effect degrades to a plain page.
   ========================================================================== */
(() => {
  'use strict';
  ...
  /* ---------------------------------------------------------------- 1. dock
     Concept-only control: swap the display face so the type stance can be
     judged as an A/B instead of argued about. Would not ship. */
  const faceButtons = qa('[data-face-set]');
  faceButtons.forEach((btn) => {
    btn.addEventListener('click', () => {
      doc.dataset.face = btn.dataset.faceSet;
  ...
  const onScroll = () => {
    ...
    if (current !== here) {
      if (railLinks.has(here)) railLinks.get(here).classList.remove('is-here');
      if (railLinks.has(current)) railLinks.get(current).classList.add('is-here');
      here = current;
    }
  ...
  /* The city plate lives in capycity.js now: concept 03 mounts itself into
     #city-stage, reads its palette out of editorial.css, and flies the camera
     down the avenue on page scroll. None of that belongs in here. */
})();
